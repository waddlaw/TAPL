...
（例）
`$d = -2$` のとき， `$\mathtt{t}$` の自由変数は `$2$` 以上（ `$\mathtt{t}=\lambda.\lambda.\mathtt{2},\ \lambda.\lambda.\mathtt{0}\ \mathtt{2}$` など）
```mathjaxBlock
\begin{align}
\uparrow_{c}^{-2}(\lambda.\lambda.\mathtt{2}) = \lambda.\lambda.\mathtt{0}
\end{align}
```
...
```mathjaxBlock
\begin{align}
removenames_{\Gamma}([\mathtt{x} \mapsto \mathtt{s}]\mathtt{t}) = [removenames_{\Gamma}(\mathtt{x}) \mapsto removenames_{\Gamma}(\mathtt{s})]removenames(\mathtt{t})
\end{align}
```
...
```mathjaxBlock
\begin{align}
\Gamma = \mathtt{a},\mathtt{b}
\end{align}
```

1. `$[\mathtt{b} \mapsto \mathtt{a}] (\mathtt{b}\ (\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{b}))$`
    - 名無し項への変換
    ```mathjaxBlock
    \begin{align}
    removenames_{\Gamma}([\mathtt{b} \mapsto \mathtt{a}] (\mathtt{b}\ (\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{b})))& = [removenames_{\Gamma}(\mathtt{b}) \mapsto removenames_{\Gamma}(\mathtt{a})] removenames_{\Gamma}(\mathtt{b}\ (\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{b})) \\
                                                                                                                        & = [\mathtt{0} \mapsto \mathtt{1}] removenames_{\Gamma}(\mathtt{b}\ (\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{b}))                                             \\
                                                                                                                        & = [\mathtt{0} \mapsto \mathtt{1}] (removenames_{\Gamma}(\mathtt{b})\ removenames_{\Gamma}(\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{b}))                       \\
                                                                                                                        & = [\mathtt{0} \mapsto \mathtt{1}] (\mathtt{0}\ (\lambda.removenames_{\Gamma,\mathtt{x}}(\lambda\mathtt{y}.\mathtt{b})))                                          \\
                                                                                                                        & = [\mathtt{0} \mapsto \mathtt{1}] (\mathtt{0}\ (\lambda.\lambda.removenames_{\Gamma,\mathtt{x},\mathtt{y}}(\mathtt{b})))                                         \\
                                                                                                                        & = [\mathtt{0} \mapsto \mathtt{1}] (\mathtt{0}\ (\lambda.\lambda.\mathtt{2}))
    \end{align}
    ```
    - 代入（名無し項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{0} \mapsto \mathtt{1}] (\mathtt{0}\ (\lambda.\lambda.\mathtt{2}))& = ([\mathtt{0} \mapsto \mathtt{1}] \mathtt{0})\ ([\mathtt{0} \mapsto \mathtt{1}](\lambda.\lambda.\mathtt{2})) \\
                                                                              & = \mathtt{1}\ (\lambda.[\mathtt{0}+1 \mapsto \uparrow_{0}^{1}(\mathtt{1})](\lambda.\mathtt{2}))               \\
                                                                              & = \mathtt{1}\ (\lambda.[\mathtt{1} \mapsto \mathtt{2}](\lambda.\mathtt{2}))                                   \\
                                                                              & = \mathtt{1}\ (\lambda.\lambda.[\mathtt{1}+1 \mapsto \uparrow_{0}^{1}(\mathtt{2})](\mathtt{2}))               \\
                                                                              & = \mathtt{1}\ (\lambda.\lambda.[\mathtt{2} \mapsto \mathtt{3}](\mathtt{2}))                                   \\
                                                                              & = \mathtt{1}\ (\lambda.\lambda.\mathtt{3})
    \end{align}
    ```
    - 代入（通常の項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{b} \mapsto \mathtt{a}] (\mathtt{b}\ (\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{b}))& = [\mathtt{b} \mapsto \mathtt{a}](\mathtt{b})\ [\mathtt{b} \mapsto \mathtt{a}](\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{b}) \\
                                                                                                  & = \mathtt{a}\ (\lambda\mathtt{x}.[\mathtt{b} \mapsto \mathtt{a}](\lambda\mathtt{y}.\mathtt{b}))                                \\
                                                                                                  & = \mathtt{a}\ (\lambda\mathtt{x}.\lambda\mathtt{y}.[\mathtt{b} \mapsto \mathtt{a}](\mathtt{b}))                                \\
                                                                                                  & = \mathtt{a}\ (\lambda\mathtt{x}.\lambda\mathtt{y}.\mathtt{a})
    \end{align}
    ```
2. `$[\mathtt{b} \mapsto \mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a})] (\mathtt{b}\ (\lambda\mathtt{x}.\mathtt{b}))$`
    - 名無し項への変換
    ```mathjaxBlock
    \begin{align}
    removenames_{\Gamma}([\mathtt{b} \mapsto \mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a})] (\mathtt{b} (\lambda\mathtt{x}.\mathtt{b})))& = [removenames_{\Gamma}(\mathtt{b}) \mapsto removenames_{\Gamma}(\mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a}))] removenames_{\Gamma}(\mathtt{b}\ (\lambda\mathtt{x}.\mathtt{b})) \\
                                                                                                                                     & = [\mathtt{0} \mapsto removenames_{\Gamma}(\mathtt{a})\ removenames_{\Gamma}(\lambda\mathtt{z}.\mathtt{a}))] removenames_{\Gamma}(\mathtt{b}\ (\lambda\mathtt{x}.\mathtt{b}))  \\
                                                                                                                                     & = [\mathtt{0} \mapsto \mathtt{1}\ (\lambda.removenames_{\Gamma,\mathtt{z}}(\mathtt{a}))] removenames_{\Gamma}(\mathtt{b}\ (\lambda\mathtt{x}.\mathtt{b}))                      \\
                                                                                                                                     & = [\mathtt{0} \mapsto \mathtt{1}\ (\lambda.\mathtt{2})] removenames_{\Gamma}(\mathtt{b}\ (\lambda\mathtt{x}.\mathtt{b}))                                                       \\
                                                                                                                                     & = [\mathtt{0} \mapsto \mathtt{1}\ (\lambda.\mathtt{2})] (removenames_{\Gamma}(\mathtt{b}) removenames_{\Gamma}(\lambda\mathtt{x}.\mathtt{b}))                                  \\
                                                                                                                                     & = [\mathtt{0} \mapsto \mathtt{1}\ (\lambda.\mathtt{2})] (\mathtt{0}\ (\lambda.removenames_{\Gamma,\mathtt{x}}(\mathtt{b}))                                                     \\
                                                                                                                                     & = [\mathtt{0} \mapsto \mathtt{1}\ (\lambda.\mathtt{2})] (\mathtt{0}\ (\lambda.\mathtt{1}))
    \end{align}
    ```
    - 代入（名無し項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{0} \mapsto \mathtt{1}\ (\lambda.\mathtt{2})] (\mathtt{0}\ (\lambda.\mathtt{1}))& = ([\mathtt{0} \mapsto \mathtt{1}\ (\lambda.\mathtt{2})] \mathtt{0})\ ([\mathtt{0} \mapsto \mathtt{1}\ (\lambda.\mathtt{2})] (\lambda.\mathtt{1})) \\
                                                                                            & = (\mathtt{1}\ (\lambda.\mathtt{2}))\ (\lambda.[\mathtt{1} \mapsto \uparrow_{0}^{1}(\mathtt{1}\ (\lambda.\mathtt{2}))] \mathtt{1})                 \\
                                                                                            & = (\mathtt{1}\ (\lambda.\mathtt{2}))\ (\lambda.[\mathtt{1} \mapsto \uparrow_{0}^{1}(\mathtt{1})\ \uparrow_{0}^{1}(\lambda.\mathtt{2})] \mathtt{1}) \\
                                                                                            & = (\mathtt{1}\ (\lambda.\mathtt{2}))\ (\lambda.[\mathtt{1} \mapsto \mathtt{2}\ (\lambda.\uparrow_{1}^{1}(\mathtt{2}))] \mathtt{1})                 \\
                                                                                            & = (\mathtt{1}\ (\lambda.\mathtt{2}))\ (\lambda.[\mathtt{1} \mapsto \mathtt{2}\ (\lambda.\mathtt{3})] \mathtt{1})                                   \\
                                                                                            & = (\mathtt{1}\ (\lambda.\mathtt{2}))\ (\lambda.\mathtt{2}\ (\lambda.\mathtt{3}))
    \end{align}
    ```
    - 代入（通常の項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{b} \mapsto \mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a})] (\mathtt{b}\ (\lambda\mathtt{x}.\mathtt{b}))& = ([\mathtt{b} \mapsto \mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a})] \mathtt{b})\ ([\mathtt{b} \mapsto \mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a})](\lambda\mathtt{x}.\mathtt{b})) \\
                                                                                                                & = (\mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a}))\ (\lambda\mathtt{x}.[\mathtt{b} \mapsto \mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a})] \mathtt{b})                                  \\
                                                                                                                & = (\mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a}))\ (\lambda\mathtt{x}.\mathtt{a}\ (\lambda\mathtt{z}.\mathtt{a}))
    \end{align}
    ```

3. `$[\mathtt{b} \mapsto \mathtt{a}] (\lambda\mathtt{b}. (\mathtt{b}\ \mathtt{a}))$`
    - 名無し項への変換
    ```mathjaxBlock
    \begin{align}
    removenames_{\Gamma}([\mathtt{b} \mapsto \mathtt{a}] (\lambda\mathtt{b}. (\mathtt{b}\ \mathtt{a})))& = [removenames_{\Gamma}(\mathtt{b}) \mapsto removenames_{\Gamma}(\mathtt{a})] removenames_{\Gamma}(\lambda\mathtt{b}. (\mathtt{b}\ \mathtt{a})) \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] removenames_{\Gamma}(\lambda\mathtt{b}. (\mathtt{b}\ \mathtt{a}))                                             \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] (\lambda.removenames_{\Gamma,\mathtt{b}'} (\mathtt{b'}\ \mathtt{a}))                                          \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] (\lambda.(removenames_{\Gamma,\mathtt{b}'}(\mathtt{b'})\ removenames_{\Gamma,\mathtt{b}'}(\mathtt{a})))      \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] (\lambda.\mathtt{0}\ \mathtt{2})
    \end{align}
    ```
    - 代入（名無し項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{0} \mapsto \mathtt{1}] (\lambda.\mathtt{0}\ \mathtt{2})& = \lambda.[\mathtt{0}+1 \mapsto \uparrow_{0}^{1}(\mathtt{0})](\mathtt{0}\ \mathtt{2})            \\
                                                                    & = \lambda.[\mathtt{1} \mapsto \mathtt{2}](\mathtt{0}\ \mathtt{2})                                \\
                                                                    & = \lambda.([\mathtt{1} \mapsto \mathtt{2}]\mathtt{0}\ [\mathtt{1} \mapsto \mathtt{2}]\mathtt{2}) \\
                                                                    & = \lambda.(\mathtt{0}\ \mathtt{2})
    \end{align}
    ```
    - 代入（通常の項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{b} \mapsto \mathtt{a}] (\lambda\mathtt{b}. (\mathtt{b}\ \mathtt{a})) &= \lambda\mathtt{b}. (\mathtt{b}\ \mathtt{a})
    \end{align}
    ```
4. `$[\mathtt{b} \mapsto \mathtt{a}] (\lambda.\mathtt{a}\ (\mathtt{b}\ \mathtt{a}))$`
    - 名無し項への変換
    ```mathjaxBlock
    \begin{align}
    removenames_{\Gamma}([\mathtt{b} \mapsto \mathtt{a}] (\lambda\mathtt{a}. (\mathtt{b}\ \mathtt{a})))& = [removenames_{\Gamma}(\mathtt{b}) \mapsto removenames_{\Gamma}(\mathtt{a})] removenames_{\Gamma}(\lambda\mathtt{a}. (\mathtt{b}\ \mathtt{a})) \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] removenames_{\Gamma}(\lambda\mathtt{a}. (\mathtt{b}\ \mathtt{a}))                                            \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] (\lambda.removenames_{\Gamma,\mathtt{a}'}(\mathtt{b}\ \mathtt{a'}))                                          \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] (\lambda.(removenames_{\Gamma,\mathtt{a}'}(\mathtt{b})\ removenames_{\Gamma,\mathtt{a}'}(\mathtt{a'})))      \\
                                                                                                       & = [\mathtt{0} \mapsto \mathtt{1}] (\lambda.\mathtt{1}\ \mathtt{0})
    \end{align}
    ```
    - 代入（名無し項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{0} \mapsto \mathtt{1}] (\lambda.\mathtt{1}\ \mathtt{0})& = \lambda.[\mathtt{0}+1 \mapsto \uparrow_{0}^{1}(\mathtt{1})](\mathtt{1}\ \mathtt{0})                \\
                                                                    & = \lambda.[\mathtt{1} \mapsto \mathtt{2}](\mathtt{1}\ \mathtt{0})                                    \\
                                                                    & = \lambda.(([\mathtt{1} \mapsto \mathtt{2}]\mathtt{1})\ ([\mathtt{1} \mapsto \mathtt{2}]\mathtt{0})) \\
                                                                    & = \lambda.(\mathtt{2}\ \mathtt{0})
    \end{align}
    ```
    - 代入（通常の項）
    ```mathjaxBlock
    \begin{align}
    [\mathtt{b} \mapsto \mathtt{a}] (\lambda\mathtt{a}. (\mathtt{b}\ \mathtt{a}))& = [\mathtt{b} \mapsto \mathtt{a}] (\lambda\mathtt{a}'. (\mathtt{b}\ \mathtt{a}'))                                  \\
                                                                                 & = \lambda\mathtt{a}'. [\mathtt{b} \mapsto \mathtt{a}](\mathtt{b}\ \mathtt{a}')                                    \\
                                                                                 & = \lambda\mathtt{a}'. ([\mathtt{b} \mapsto \mathtt{a}](\mathtt{b})\ [\mathtt{b} \mapsto \mathtt{a}](\mathtt{a}')) \\
                                                                                 & = \lambda\mathtt{a}'. (\mathtt{a}\ \mathtt{a}')
    \end{align}
    ```
...
`$\mathtt{s}$` と `$\mathtt{t}$` は `$n$` 項であり `$\mathtt{j} \leq n$` ならば `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}$` は `$n$` 項である．

**解答**

引数 `$\mathtt{t}$` の帰納法によって示す．
- `$\mathtt{t} = \mathtt{k}$` （自由変数）のとき
    ```mathjaxBlock
\begin{align}
[\mathtt{j} \mapsto \mathtt{s}] \mathtt{k} &=
\left\{
...
\end{align}
    ```
    - `$\mathtt{k} =    \mathtt{j}$` のとき，
      `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{k} = \mathtt{s}$` となり， `$\mathtt{s}$` は `$n$` 項である
    - `$\mathtt{k} \neq \mathtt{j}$` のとき，
      `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{k} = \mathtt{k}$` となり， `$\mathtt{k}$` は `$n$` 項である
- `$\mathtt{t} = \lambda.\mathtt{t}_{1}$` のとき
    ```mathjaxBlock
\begin{align}
[\mathtt{j} \mapsto \mathtt{s}] (\lambda.\mathtt{t}_{1}) = \lambda.[\mathtt{j}+1 \mapsto \uparrow^{1}(\mathtt{s})]\mathtt{t}_{1}
\end{align}
    ```
    - 帰納法の仮定
        - （ `$\mathtt{s}$` と `$\mathtt{t}_{1}$` は `$n$` 項であり `$\mathtt{j} \leq n$` ならば
          `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{1}$` は `$n$` 項である ）
        - `$\mathtt{s}$` と `$\mathtt{t}_{1}$` は `$n+1$` 項であり `$\mathtt{j} \leq n+1$` ならば
          `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{1}$` は `$n+1$` 項である
    帰納法の仮定より， `$[\mathtt{j}+1 \mapsto \uparrow^{1}(\mathtt{s})]\mathtt{t}_{1} \in \mathcal{T}_{n+1}$` である．
    よって，定義 6.1.2 の条件 2 より
    ```mathjaxBlock
\begin{align}
[\mathtt{j}+1 \mapsto \uparrow^{1}(\mathtt{s})]\mathtt{t}_{1} \in \mathcal{T}_{n+1} \wedge n > 0 \Rightarrow \lambda.[\mathtt{j}+1 \mapsto \uparrow^{1}(\mathtt{s})]\mathtt{t}_{1} \in \mathcal{T}_{n}
\end{align}
    ```
    となるので， `$n$` 項である．

- `$\mathtt{t} = \mathtt{t}_{1}\ \mathtt{t}_{2}$` のとき
    ```mathjaxBlock
\begin{align}
[\mathtt{j} \mapsto \mathtt{s}] (\mathtt{t}_{1}\ \mathtt{t}_{2}) = ([\mathtt{j} \mapsto \mathtt{s}]\mathtt{t}_{1})\ ([\mathtt{j} \mapsto \mathtt{s}]\mathtt{t}_{2})
\end{align}
    ...
    - 帰納法の仮定
        - `$\mathtt{s}$` と `$\mathtt{t}_{1}$` は `$n$` 項であり `$\mathtt{j} \leq n$` ならば
          `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{1}$` は `$n$` 項である
        - `$\mathtt{s}$` と `$\mathtt{t}_{2}$` は `$n$` 項であり `$\mathtt{j} \leq n$` ならば
          `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{2}$` は `$n$` 項である
    帰納法の仮定より， `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{1} \in \mathcal{T}_{n}$`, `$[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{2} \in \mathcal{T}_{n}$` である．
    よって，定義 6.1.2 の条件 3 より
    ```mathjaxBlock
\begin{align}
[\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{1} \in \mathcal{T}_{n} \wedge [\mathtt{j} \mapsto \mathtt{s}] \mathtt{t}_{2} \in \mathcal{T}_{n} \Rightarrow (\mathtt{t}_{1}\ \mathtt{t}_{2}) \in \mathcal{T}_{n}
\end{align}
    ```
    となるので， `$n$` 項である．
...
\begin{align}
\begin{array}{rll}
\mathtt{t}                   & \xrightarrow{removenames}& {\mathtt{t}'}_{1}             \\
代入（定義 5.3.5） \downarrow&                          & \downarrow 代入（定義 6.2.4） \\
{\mathtt{t}'}_{2}            & \xrightarrow{removenames}& {\mathtt{t}'}
\end{array}
\end{align}
```
...
```mathjaxBlock
\begin{align}
removenames_{\Gamma}([\mathtt{x} \mapsto \mathtt{s}] \mathtt{t}) = [{\Gamma}(\mathtt{x}) \mapsto removenames_{\Gamma}(\mathtt{s})](removenames_{\Gamma}(\mathtt{t})
\end{align}
```

## 6.3 評価
名無し項の評価関係を定義するために，ベータ簡約の規則のみを変更する

**束縛変数の消費**

`$((\lambda\mathtt{x}.\ \mathtt{t}_{12}) \mathtt{v}_{2})$` を `$[\mathtt{x} \mapsto \mathtt{v}_{2}] \mathtt{t}_{12}$` と簡約する．

（例）仮に `$\mathtt{t}_{12} = \mathtt{x}$` とすると，
```mathjaxBlock
\begin{align}
                    & (\lambda\mathtt{x}.\ \mathtt{x}) \mathtt{v}_{2} \\
\longrightarrow\quad& [\mathtt{x} \mapsto \mathtt{v}_{2}] \mathtt{x}
\end{align}
```
となり，束縛変数 `$\mathtt{x}$` が消える．

**解決策**

代入結果の中の変数に対して，再度番号を振る．

（例）
```mathjaxBlock
\begin{align}
                                              & (\lambda.\mathtt{1}\ \underline{\mathtt{0}}\ \mathtt{2})\ \underline{(\lambda.\mathtt{0})} \\
\longrightarrow_{代入}                   \quad& \mathtt{1}\ (\lambda.\mathtt{0})\ \mathtt{2}                                               \\
\longrightarrow_{\lambda が外れたので -1}\quad& \mathtt{0}\ (\lambda.\mathtt{0})\ \mathtt{1}
\end{align}
```

（例） `$\mathtt{v}_{2}$` に自由変数が含まれている場合
```mathjaxBlock
\begin{align}
                                              & (\lambda.\mathtt{1}\ \mathtt{0}\ \mathtt{2})\ (\lambda.\mathtt{0}\ \mathtt{1})                                                               \\
\longrightarrow\quad                          & \mathtt{1}\ (\lambda.\mathtt{0}\ \mathtt{1})\ \mathtt{2}                                                                                     \\
\longrightarrow\quad                          & \uparrow^{-1}(\mathtt{1}\ (\lambda.\mathtt{0}\ \mathtt{1})\ \mathtt{2})                                                                      \\
\longrightarrow\quad                          & \uparrow^{-1}(\mathtt{1})\ \uparrow^{-1}(\lambda.\mathtt{0}\ \mathtt{1})\ \uparrow^{-1}(\mathtt{2})                                          \\
\longrightarrow\quad                          & \mathtt{0}\ (\lambda.\uparrow_{1}^{-1}(\mathtt{0}\ \mathtt{1}))\ \mathtt{1}                                                                  \\
\longrightarrow\quad                          & \mathtt{0}\ (\lambda.\uparrow_{1}^{-1}(\mathtt{0})\ \underline{\uparrow_{1}^{-1}(\mathtt{1})})\ \mathtt{1}                                   \\
\longrightarrow\quad                          & \mathtt{0}\ (\lambda.\mathtt{0}\ \underline{\mathtt{0}})\ \mathtt{1}                                      & 自由変数なのに束縛されてしまった
\end{align}
```
したがって， `$\uparrow^{1}\mathtt{v}_{2}$` を先に行ってから，代入した後の項に `$\uparrow^{-1}$` をする．

**ベータ簡約規則**
```mathjaxBlock
\begin{align}
(\lambda.\mathtt{t}_{12})\ \mathtt{v}_{2}& \quad\longrightarrow\quad \uparrow^{-1}([\mathtt{0} \mapsto \uparrow^{1}(\mathtt{v}_{2})] \mathtt{t}_{12})&&& (\mbox{E-AppAbs})
\end{align}
```
...
E-AppAbs の定義の `$\uparrow^{-1}([\mathtt{0} \mapsto \uparrow^{1}(\mathtt{v}_{2})] \mathtt{t}_{12})$`より
`$\uparrow^{-1}$` によって負のインデックスが出現するためには，
`$([\mathtt{0} \mapsto \uparrow^{1}(\mathtt{v}_{2})] \mathtt{t}_{12})$` の項のどこかに `$\mathtt{0}$` が存在する必要がある．

しかし， `$\mathtt{t}_{12}$` の `$\mathtt{0}$` の出現は全て `$\uparrow^{1}(\mathtt{v})$` で置き換えられるため
`$\mathtt{0}$` が出現することはない．

また， `$\uparrow^{1}(\mathtt{v})$` は `$\uparrow_{0}^{1}(\mathtt{v})$` なので，
`$\mathtt{v}$` のインデックスは全て `$0$` より大きい．

したがって，E-AppAbs にある負のシフト操作によって負のインデックスが作られることはない．
