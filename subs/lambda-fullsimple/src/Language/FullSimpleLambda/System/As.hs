...
instance System As where
  ...
  data Term As
    = TmVar Int -- ^ 変数
    | TmLam VarName (Ty As) (Term As) -- ^ ラムダ抽象
    | TmApp (Term As) (Term As) -- ^ 関数適用
    | TmAscribe (Term As) (Ty As) -- ^ 11.4   型指定
    deriving (Show, Eq)

  data Ty As
    = TyArr (Ty As) (Ty As) -- ^ 関数の型
    | TyUnit -- ^ Unit 型
    deriving (Show, Eq)

  data Context As
    = CtxEmpty -- ^ 空の文脈
    | CtxVar (Context As) VarName (Ty As) -- ^ 項変数の束縛
    deriving (Show, Eq)
