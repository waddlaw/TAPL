...
解答: `$removenames_{\Gamma}(\mathtt{t})$` の定義
```mathjaxBlock
\begin{align}
...
解答: `$restorenames_{\Gamma}(\mathtt{t})$` の定義
```mathjaxBlock
\begin{align}
...
任意の名無し項 `$\mathtt{t}$` について
```mathjaxBlock
\begin{align}
removenames_{\Gamma}(restorenames_{\Gamma}(\mathtt{t})) = \mathtt{t}
\end{align}
```
を証明する．
...
&\Leftrightarrow&& 帰納法の仮定より \lambda.\mathtt{t}_{1} = \lambda.\mathtt{t}_{1}
\end{align}
```
...
&\Leftrightarrow&& 帰納法の仮定より \mathtt{t}_{1}\ \mathtt{t}_{2} = \mathtt{t}_{1}\ \mathtt{t}_{2}
\end{align}
```
